...
fun main() {
    ...
    """------------------------------------------------------------------------------------------------------------------"""
    witArgs("common","common"){a,b -> "Hello World $a $b"} // 이런식으로 간략화 하기 위해서는 람다식 함수가 마지막 배개변수로 위치해야함
    witArgs("common", "common", {a,b -> "Hello World $a $b"})
    """------------------------------------------------------------------------------------------------------------------"""
    twoLambda({a,b -> "Hello World $a, $b"},{"Hello World $it"}) //람다식 두개 이상이 매개 변수로 들어간다면 소괄호는 생략이 불가능
